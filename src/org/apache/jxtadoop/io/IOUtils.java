...
package org.apache.jxtadoop.io;
...
public class IOUtils {
  ...
  public static void copyBytes(InputStream in, OutputStream out, Configuration conf)
    throws IOException {
    copyBytes(in, out, conf.getInt("io.file.buffer.size", P2PConstants.IO_FILE_BUFFER_SIZE), true);
  }
  ...
  public static void copyBytes(InputStream in, OutputStream out, Configuration conf, boolean close)
    throws IOException {
    copyBytes(in, out, conf.getInt("io.file.buffer.size", P2PConstants.IO_FILE_BUFFER_SIZE),  close);
  }
  ...
  public static void closeStream( java.io.Closeable stream ) {
	LOG.debug("call to IOUtils closeStream()");
    cleanup(null, stream);
  }
  ...
  public static void closeSocket( JxtaSocket jsock ) {
    ...
    if ( jsock != null ) {
      try {  
    	  jsock.shutdownInput();
    	  jsock.shutdownOutput();
    	  
    	  LOG.debug("jsock connected : "+jsock.isConnected());
  ...
  public static void closeSocket( Socket sock ) {
	  LOG.debug("call to IOUtils closeSocket()");
    // avoids try { close() } dance
    if ( sock != null ) {
      try {  
    	  sock.close();
